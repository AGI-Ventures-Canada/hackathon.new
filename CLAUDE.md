...
| developer.chrome.com WebMCP | CLAUDE.md |
...
- Use job-to-be-done labels ("Manage hackathons from your AI agent" not "Install hackathon.new Skills")
...
**Sandbox exception:** If running inside a sandboxed Codex environment, do not open Chrome, launch `agent-browser`, or run `bun run browser`. Use automated tests/builds instead and note that browser verification was skipped because the sandbox blocks browser startup.
...
| `WEBMCP_ORIGIN_TRIAL_TOKEN` | Optional exact-origin token that enables WebMCP in enrolled Vercel preview and production origins |

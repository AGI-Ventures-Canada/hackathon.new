...
Currently configured project: `hackathon.new` (target: `..`).

## Setup

`bunx deepsec init` created this workspace and normally completes its
...
If setup was interrupted, run `bun run deepsec setup` here or re-run the
original init command. Checkpoints in `data/oatmeal/setup/setup-state.json`
...
bun run deepsec process     --concurrency 5
bun run deepsec revalidate  --concurrency 5                  # cuts FP rate
...
`--project-id oatmeal` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/oatmeal/`.
...
```bash
bun run deepsec init-project ../some-other-package   # path relative to .deepsec/
```

...
describe("DirectWebMcpTools", () => {
  it("executes directly without rendering a confirmation dialog and stays registered across refreshes", async () => {
    ...
    const result = await execute.execute({ actionRef: "action-1", body: '{"name":"Synthetic event"}', requestKey: "create-test-event" })
  ...
  it("invalidates old actions and results when the organization changes or user signs out", async () => {
    ...
    await old.execute({ actionRef: "action-1", requestKey: "create-old-org" })
    clerkState.orgId = "org-two"
    rerender(<DirectWebMcpTools />)
    expect(await old.execute({ actionRef: "action-1", requestKey: "create-old-again" })).toMatchObject({ ok: false, error: { code: "session_changed" } })

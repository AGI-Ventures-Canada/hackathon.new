...
export async function getBatchJudgeStats(
  ...
  type StatsEvent = JudgingWindowEvent & { id: string; status: string; phase: string | null; results_published_at: string | null }
  type StatsRound = JudgingWindowRound & { id: string; hackathon_id: string; status: string }
  const events = new Map((eventResult.data as StatsEvent[] | null ?? []).map((event) => [event.id, event]))
  ...
  for (const [eventId, stats] of result) {
    ...
    stats.actionableAssignments = !event || !ready ? 0 : countActionableJudgeReviews(assignments.filter((assignment) => {
      ...
      return canWriteJudgingWindow(event, round)
    }).map(toReview))
  }
